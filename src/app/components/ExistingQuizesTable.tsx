  ...
  return (
    <div className="min-h-full bg-slate-50">
      <div className="max-w-7xl mx-auto px-6 pt-4 pb-10">
                    ...
                    <tr key={q.id} className="border-b border-slate-100 last:border-b-0 hover:bg-slate-50">
                      ...
                      <td className="px-6 py-3 text-sm text-slate-700 border-r border-slate-100">
                        <div className="flex items-center gap-2">
                          <span className="tabular-nums">{q.questionCount}</span>
                          <button
                            type="button"
                            onClick={() => setPreviewQuiz(q.id)}
                      ...
                      <td className="px-6 py-3 text-sm text-slate-600 border-r border-slate-100">{formatDate(q.createdAt)}</td>
                      <td className="px-6 py-3">
                        <Link
                          href={`/academy/ebright-class-syllabus/user/quiz/answer/${q.id}`}
                          title="Answer Quiz"
        ...
        {previewQuiz && (() => {
          const q = quizzes.find((x) => x.id === previewQuiz);
          if (!q) return null;
          return (
            <div
                ...
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      {q.questionCount} question{q.questionCount === 1 ? "" : "s"} · {q.status}
                    </p>
                  </div>
                  <span
