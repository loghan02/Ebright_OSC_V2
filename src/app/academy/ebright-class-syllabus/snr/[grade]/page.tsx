...
export async function generateMetadata({ params }: { params: Promise<{ grade: string }> }) {
  ...
  return { title: `SNR ${gradeLabel(grade)}` };
}

export default async function SnrGradePage({ params }: { params: Promise<{ grade: string }> }) {
  const { grade } = await params;
        ...
        <div className="max-w-7xl mx-auto px-6 pt-4 pb-10">
          <Breadcrumb
            items={[
              ...
              { label: "SNR", href: "/academy/ebright-class-syllabus/snr" },
              { label: label },
            ]}
          />
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 md:p-8">
            ...
            <ul className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
              {chapters.map((c) => (
                <li key={c}>
                  <Link
                    href={`/academy/ebright-class-syllabus/snr/${grade}/chapter-${c}`}
                    className="group block bg-amber-200 hover:bg-amber-300 rounded-2xl py-8 px-4 text-center transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-600"
                  >
                    <div className="text-5xl mb-3" aria-hidden="true">📘</div>
                    <div className="font-bold tracking-wide text-amber-900">
                      G{n} C{c}
                    </div>
                  </Link>
                </li>
