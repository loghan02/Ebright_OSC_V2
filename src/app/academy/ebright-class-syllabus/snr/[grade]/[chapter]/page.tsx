...
export async function generateMetadata({ params }: { params: Promise<RouteParams> }) {
  ...
  return { title: `SNR G${g} C${c}` };
}

export default async function SnrChapterPage({ params }: { params: Promise<RouteParams> }) {
  const { grade, chapter } = await params;
  const g = grade.match(GRADE_RE)?.[1];
  const c = chapter.match(CHAPTER_RE)?.[1];
  if (!g || !c) notFound();
  ...
  const label = `G${g} C${c}`;
  const backHref = `/academy/ebright-class-syllabus/snr/${grade}`;
  const storageKey = `academy.snr.${grade}.${chapter}`;
