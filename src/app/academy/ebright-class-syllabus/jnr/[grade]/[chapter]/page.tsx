...
export default async function ChapterPage({ params }: { params: Promise<RouteParams> }) {
  ...
  const label = `G${g} C${c}`;
  const backHref = `/academy/ebright-class-syllabus/jnr/${grade}`;
  const storageKey = `academy.jnr.${grade}.${chapter}`;
  const gradeLabel = grade.replace("-", " ").replace(/\b\w/g, (ch) => ch.toUpperCase());
    ...
    <AppShell email={userEmail} role={userRole} name={userName}>
      <ChapterCanvas
        ...
        breadcrumb={[
          ...
          { label: gradeLabel, href: backHref },
          { label },
        ]}
      />
    </AppShell>
