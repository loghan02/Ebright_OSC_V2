...
export async function generateMetadata({ params }: { params: Promise<{ grade: string }> }) {
  ...
  return { title: `JNR ${gradeLabel(grade)}` };
}

export default async function JnrGradePage({ params }: { params: Promise<{ grade: string }> }) {
  const { grade } = await params;
        ...
        <div className="max-w-7xl mx-auto px-6 pt-4 pb-10">
          <Breadcrumb
            items={[
              ...
              { label: "JNR", href: "/academy/ebright-class-syllabus/jnr" },
              { label: label },
            ]}
          />
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 md:p-8">
            ...
            <ul className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
              {chapters.map((c) => (
                <li key={c}>
                  <Link
                    href={`/academy/ebright-class-syllabus/jnr/${grade}/chapter-${c}`}
                  ...
                  >
                    ...
                    <div className="font-bold tracking-wide text-emerald-900">
                      G{n} C{c}
                    </div>
                  </Link>
                </li>
